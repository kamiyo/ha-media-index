param(
    [string]$DestPath = "\\10.0.0.62\config\custom_components\media_index",
    [string]$VerifyEntity,
    [switch]$DumpErrorLog,
...
$src = Join-Path $repoRoot "custom_components\media_index"

# Track verification failure to signal regressions via exit code
$script:mediaIndexFail = $false
$componentCopied = $false
$restartAttempted = $false
$restartAccepted = $false
$sawDowntime = $false

Write-Host "Deploying Media Index Integration from: $src" -ForegroundColor Cyan
Write-Host "To: $DestPath" -ForegroundColor Cyan

if (-not (Test-Path $src)) {
    Write-Warning "Source path not found: $src"
    Write-Warning "Expected structure: custom_components\media_index\"
    exit 0
}
...
if (-not [string]::IsNullOrWhiteSpace($baseUrl) -and -not [string]::IsNullOrWhiteSpace($token)) {
    ...
    function Get-HAErrorLogTail {
        ...
        try {
            ...
            $mediaIndexLines = $lines | Where-Object { $_ -match 'custom_components\.media_index|media_index' }
            if ($mediaIndexLines -and $mediaIndexLines.Count -gt 0) {
                $mediaIndexTail = ($mediaIndexLines | Select-Object -Last 120) -join "`n"
                if ($mediaIndexTail) { Write-Host "--- Media Index related log lines (last 120) ---`n$mediaIndexTail`n--- end ---" -ForegroundColor Yellow }
            } else {
                Write-Host "No Media Index specific lines found in the recent log tail." -ForegroundColor DarkGray
            }
            if ($env:WM_SAVE_ERROR_LOG_TO_TEMP -match '^(1|true|yes)$') {
                $ts = Get-Date -Format 'yyyyMMdd-HHmmss'
                $tmpFile = Join-Path $env:TEMP "ha-error-log-media-index-$ts.log"
    ...
    if ($restartAttempted -and -not $sawDowntime) {
        ...
        if ($FailOnNoRestart) { $script:mediaIndexFail = $true }
    }
    ...
    if ($back -and -not [string]::IsNullOrWhiteSpace($VerifyEntity)) {
        ...
        # Attribute verification: ensure expected attributes are present
        if ($ok) {
            try {
                ...
                if ($stateResp2.StatusCode -eq 200) {
                    ...
                    $hasScanStatus = $attrs.PSObject.Properties.Name -contains 'scan_status'
                    $hasLastScan = $attrs.PSObject.Properties.Name -contains 'last_scan_time'
                    $hasFolders = $attrs.PSObject.Properties.Name -contains 'total_folders'
                    if (-not $hasScanStatus -or -not $hasLastScan -or -not $hasFolders) {
                        Write-Warning "Attribute verification failed: missing expected attributes on $VerifyEntity (scan_status/last_scan_time/total_folders)."
                        $script:mediaIndexFail = $true
                        Get-HAErrorLogTail -why 'attribute verification failed (attributes missing or not populated)'
                    } else {
                        Write-Host "Attribute verification passed for $VerifyEntity (scan_status=$($attrs.scan_status), last_scan_time=$($attrs.last_scan_time), total_folders=$($attrs.total_folders))." -ForegroundColor DarkGray
                    }
                }
            } catch {
